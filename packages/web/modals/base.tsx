...
export const ModalBase = ({
  ...
  // Remove custom body class on unmount
  useUnmount(() => {
    if (bodyOpenClassNames) {
      document.body.classList.remove(...bodyOpenClassNames.split(" "));
    }
  });
  ...
  return (
    <ReactModal
      isOpen={isOpen}
      onRequestClose={(e) => {
        if (e?.preventDefault) e.preventDefault(); // Ensure e is defined
        if (onRequestClose) onRequestClose(); // Safe callback execution
      }}
      bodyOpenClassName={bodyOpenClassNames}
